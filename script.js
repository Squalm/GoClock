...
function updateTimer() {
    ...
    initialTime =
        (document.getElementById("initialTimeHours").value == ""
            ? 0
            : parseInt(document.getElementById("initialTimeHours").value)) *
            3600 +
        (document.getElementById("initialTimeMins").value == ""
            ? 0
            : parseInt(document.getElementById("initialTimeMins").value)) *
            60 +
        (document.getElementById("initialTimeSecs").value == ""
            ? 0
    ...
    periodTime =
        document.getElementById("periodTime").value == ""
            ? 0
            : parseInt(document.getElementById("periodTime").value);
    periodTime *= 10; // This puts the numebr in 10th of a second

    if (
        isNaN(periodTime) &&
        (timeControl == "byo-yomi" || timeControl == "fischer")
    ) {
    ...
    periodNumber =
        document.getElementById("periodNumber").value == ""
            ? 0
            : parseInt(document.getElementById("periodNumber").value);
    ...
    if (feedback == "Updated Clock.") {
        ...
        document.getElementById("leftClock").innerHTML = timeFormatConvert(
            initialTime,
            removeUnnecessaryPaddingUser
        ).toString();
        document.getElementById("rightClock").innerHTML = timeFormatConvert(
            initialTime,
            removeUnnecessaryPaddingUser
        ).toString();
...
document
    .querySelector("#updateTimerButton")
    .addEventListener("click", updateTimer);
...
function showHideInputs(evt) {
    ...
    if (_timeControl == "byo-yomi") {
        ...
        document.getElementById("periodTimeLabel").innerHTML =
            "Set a period length<br>" +
            document.getElementById("periodTime").outerHTML;
        document.getElementById("periodNumberLabel").innerHTML =
            "Set a period number<br>" +
            document.getElementById("periodNumber").outerHTML;
        ...
        document.getElementById("periodTimeLabel").innerHTML =
            ...
            document.getElementById("periodTime").outerHTML;
        document.getElementById("periodNumberLabel").innerHTML =
            "Set a number of moves per period<br>" +
            document.getElementById("periodNumber").outerHTML;
    } else {
...
document
    .querySelector("#timeControl")
    .addEventListener("change", showHideInputs);
...
function startTimer() {
    if (feedback == "Updated Clock.") {
        ...
        if (timeControl == "fischer") {
            // This fixes the bug where in fischer time gets extra bonus time as the game starts
...
function timeFormatConvert(time, removeUnnecessaryPadding) {
    ...
    if (typeof time === typeof "string") {
        return (
            ...
            parseInt(time.slice(3, 6)) * 60 +
            parseInt(time.slice(6))
        );
    }
    ...
    if (typeof time === typeof 20) {
        ...
        if (!removeUnnecessaryPadding) {
            ...
            // If removeUnnecessaryPadding is true
            ...
            if ((time / 36000) | (0 > 0)) {
                ...
                const _timeMins = (
                    ((_time - ((_time / 3600) | 0) * 3600) / 60) |
                    0 |
                    0
                )
                    .toString()
                    .padStart(2, "0");
                const _timeSecs = ((_time - ((_time / 60) | 0) * 60) | 0)
                    .toString()
                    .padStart(2, "0");
...
function allTimeControl() {
    ...
    if (activeTimer == "leftClock" && gameActive) {
        ...
        if (timeControl == "canadian") {
            if (moveCounterLeft >= periodNumber && inPeriodLeft) {
                // When using canadian time, periodNumber is the number of moves to get more time
                moveCounterLeft = 0;
                timeRemainingLeft = periodTime;
            }
        }
        ...
        timer = window.setInterval(function () {
            _timesetup = new Date();
            timeRemainingRight = Math.round(
                timeRemainingOnLastClick -
                    (_timesetup.getTime() - timeOnLastClick) / 100
            );

            // Byo-yomi rules
            if (timeControl == "byo-yomi") {
                if (
                    (periodNumberRemainingRight <= 0 ||
                        periodNumberRemainingRight <= 0) &&
                    timeRemainingRight <= 0
                ) {
                    stopTimer();
                }
        ...
        if (timeControl == "canadian") {
            if (moveCounterRight >= periodNumber && inPeriodRight) {
                // When using canadian time, periodNumber is the number of moves to get more time
                moveCounterRight = 0;
                timeRemainingRight = periodTime;
            }
...
function timeDisplay() {
    ...
    if (timeControl == "byo-yomi") {
        if (periodNumberRemainingRight > 0) {
            document.getElementById("periodMarkerRight").innerHTML =
                periodNumberRemainingRight.toString() +
                "x " +
                timeFormatConvert(periodTime, removeUnnecessaryPaddingUser);
        }
        if (periodNumberRemainingRight == 0) {
        ...
        if (periodNumberRemainingLeft > 0) {
            document.getElementById("periodMarkerLeft").innerHTML =
                periodNumberRemainingLeft.toString() +
                "x " +
                timeFormatConvert(periodTime, removeUnnecessaryPaddingUser);
        }
        if (periodNumberRemainingLeft == 0) {
        ...
        document.getElementById("periodMarkerRight").innerHTML =
            ...
            periodNumber.toString() +
            (inPeriodRight
                ? " (" + (periodNumber - moveCounterRight).toString() + ")"
                : "");
        document.getElementById("periodMarkerLeft").innerHTML =
            timeFormatConvert(periodTime, removeUnnecessaryPaddingUser) +
            " / " +
            periodNumber.toString() +
            (inPeriodLeft
                ? " (" + (periodNumber - moveCounterLeft).toString() + ")"
                : "");
    }
}
